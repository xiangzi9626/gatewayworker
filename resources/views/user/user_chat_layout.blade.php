...
<head>
    ...
    <script>
        function videoChat(){
            window.location="/user/video_chat?id={{$_GET["id"]}}&nickname={{$_GET["nickname"]}}&headImg={{$_GET["headImg"]}}";
        }
    </script>
    <script>
        var currentObj = null;
    ...
    <script>
        $(function () {
            ...
            btn.onclick = function () {
                ...
                var str = "_token={{csrf_token()}}&msg=" + encodeURIComponent(text.innerHTML) + "&receive_id={{$_GET["id"]}}";
                ajax("/user/send_user_chat", str, function (res) {
                    var json = eval("(" + res + ")");
                    if (json.type === "error") {
                        layer.msg(json.msg);
                    } else {
                ...
                switch (obj.type) {
                    ...
                    default:
                        ...
                        if (obj.uid ==={{session("user")->id}}) {
                            ...
                            str += '<tr>';
                            str += '<td style="text-align: right;" valign="middle">';
                            if (obj.type === "text") {
                                ...
                                str += '<p class="msg-box-right" style="min-width:' + w + ';text-align:right;">';
                                str += '<span style="text-align:left;display: inline-block;">';
                            ...
                            if (obj.type === "text") {
                                ...
                                str += '<p class="msg-box-left" style="min-width:' + w + '">';
                                str += '<i onclick="playAudio(' + "this,'" + obj.msg + "'" + ')" style="font-size:40px;color:purple" class="fa fa-play-circle-o"></i>';
                                str += '<span style="line-height:0;color:purple;">' + duration + 's&nbsp;</span>';
                                str += '</p>';
                            } else {
                        ...
                        ajax("/user/update_msg_status", "_token={{csrf_token()}}&send_id=" + obj.uid, function (str) {
...
<form method="post" style="display: none;" id="uploadForm" enctype="multipart/form-data">
    <input accept="image/gif,image/jpeg,image/png,image/bmp" type="file" id="uploadImg" name="upload"
           style="display:none">
</form>
<form method="post" style="display: none;" id="cameraForm" enctype="multipart/form-data">
    <input type="file" capture="camera" accept="image/*" id="uploadCamera"
...
<footer class="fixed-bottom" style="width: 100%;background:#fff" id="footer">
    ...
    <div class="container" style="padding:5px 0;border-width: 2px 2px 0 2px;border-color:grey;border-style: solid;display:none;text-align: center"
        ...
        <div class="row">
            ...
            <div onclick="videoChat();$('#moreDiv').css('display','none');" class="col-3" style="color: grey">
                <i class="fa fa-video-camera"></i><br>视频通话
            </div>
            <div onclick="alert('视频通话');$('#moreDiv').css('display','none');" class="col-3" style="color: grey">
                <i class="fa fa-volume-control-phone"></i><br>语音通话
            </div>
        </div>
        </div>
    <!-----///////---->
    <table cellpadding="0" cellspacing="0">
        <tr>
            ...
            <td style="width:50px">
                <span id="emoji" style="margin-left:5px;font-size:50px;color:#9F9F9F"><i
            ...
            <td style="width:50px;padding:0 5px;">
                ...
                <span id="more" style="margin:0 5px;font-size:50px;color:#9F9F9F;"><i
            ...
            if (duration>=12){
                ...
                recorder.exportWAV(function (blob) {
                  ...
                  $("#tips-icon").attr("src","/static/images/load.gif");
                    ...
                    fd.append("receive_id",{{$_GET["id"]}});
                    $.ajax({
                        type: "POST",
                        data: fd,
                        url: "/user/upload_user_audio?_token={{csrf_token()}}",
                        processData: false,
                        contentType: false,
...
<script>
    $(function () {
        $("#uploadImg").on("change", function () {
            ...
            formData.append("receive_id",{{$_GET["id"]}});
            $.ajax({
                type:'post',
                url:'/user/user_upload?_token={{csrf_token()}}',
                data:formData,
                processData:false,//*
                contentType:false,//*
